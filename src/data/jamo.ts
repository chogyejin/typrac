...
export function getJamoText(lang: Language, difficulty: Difficulty): string {
  if (lang === "ko") {
    const count =
      difficulty === "easy" ? 20 : difficulty === "medium" ? 28 : 36;
    if (difficulty === "easy") {
      return generateSequence(KO_CONSONANTS_BASIC, count);
    } else if (difficulty === "medium") {
      return generateSequence(
        [...KO_CONSONANTS_BASIC, ...KO_VOWELS_BASIC],
        count,
      );
    } else {
      return generateSequence(
    ...
    const count =
      difficulty === "easy" ? 24 : difficulty === "medium" ? 32 : 40;
    if (difficulty === "easy") {
